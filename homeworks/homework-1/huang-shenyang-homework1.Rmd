...
- The `sum(x)` command will throw an error as characters cannot be added together. The previous two commans will still work as R can sort and find the maximum of character arrays, however, it seems that variable `x` should have been a numeric array `c(5,12,7)` for the intended operations.
...
- Reproducible code is easier for others (or my future self) to understand, trace back any potential source of errors, and reuse for similar practices in the future.
