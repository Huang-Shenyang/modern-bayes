...
function.\footnote{This is not an approximation given that we can actually compute the integral exactly. We will use this exact expression when comparing to Monte Carlo and Importance sampling in the other tasks below.}
  ...
  Substitution: $u = x^4, x = u^{1/4} \Rightarrow du = 4x^3 dx \Rightarrow dx = \frac{1}{4u^{3/4}} du$.
  
  \begin{align*}
   & \int_{-\infty}^\infty \exp(-x^{4}) dx \\
  =& 2\int_0^\infty \exp(-x^{4}) dx \\
  =& 2\int_0^\infty \frac{\exp(-u)}{4u^{3/4}} du \\
  =& 1/2\int_0^\infty u^{1/4-1}e^{-u} du \\
  =& \frac{\Gamma(1/4)}{2} \\
  \end{align*}
...
```{r}
integrand <- function(x) {2^{-5/4} * sqrt(2*pi/abs(x))}
n <- 10e6
X <- rnorm(n) # draw samples from the standard normal distribution
values <- integrand(X) # compute integral
mean(values)
sd(values)/sqrt(n)
...
```{r}
integrand_IS <- function(x) {exp(-x^4)/dnorm(x)}
n <- 10e6
X <- rnorm(n) # draw samples from the standard normal distribution
values_IS <- integrand_IS(X) # compute integral
mean(values_IS)
sd(values_IS)/sqrt(n)
...
```{r}
integrand_Unif <- function(x) {20*exp(-x^4)}
n <- 10e6
X <- seq(-10,10, length=n) # draw samples from a uniform distribution [-10, 10]
values_Unif <- integrand_Unif(X) # compute integral
mean(values_Unif)
sd(values_Unif)/sqrt(n)
