...
```{r 1-histogram of IQ change, echo=F, include=T}
# data
...
plotdf <- data.frame(
  IQ_change = c(x, y),
  Group = c(rep('spurters', length(x)), rep('controls', length(y)))
)

# plot histogram of IQ_change for both groups
plotdf %>% 
  ggplot() +
  geom_histogram(aes(fill=Group, x=IQ_change),
                 color='gray20', 
                 binwidth = 4,
                 position = 'identity',
                 alpha = 0.5) +
  ggtitle(label='histogram of IQ score changes')+
  NULL

```

- Based on the histogram, it appears that both groups have positive changes (improvements) in their IQ scores, but spurters tend to have greater changes than controls.
...
```{r 3 plot posterior, echo=F, include=T}
# reference for the NormalGamma function: https://stats.stackexchange.com/questions/52344/sampling-from-the-normal-gamma-distribution-in-r
NormalGamma <- function(k, m, c, a, b) {
  # This function draws k samples from the NormalGamma distribution by:
  # first drawing k samples from the Gamma distribution
  # and then use each pair of parameters (mean and sd) to get k samples from the Normal distribution 
  
  # m = posterior mean for mu
  # c = sample size for estimating mu
  # a = shape for posterior on lambda
  # b = rate for posterior on lambda
  
  # Use a,b to get lambda which will be used in Normal
  lambda <- rgamma(k, a, b)
  mu <- rnorm(k, m, sqrt(1/(c*lambda))) # Note that the rnorm function takes the standard deviation, not the variance, for the Normal distribution
  
  # output: posterior samples of mu and lambda
...
k <- 10e2
# spurters
y_s <- NormalGamma(k, 24.0, 8, 4, 855.0)
# controls
y_c <- NormalGamma(k, 11.8, 49, 24.5, 6344.0)

plotdf2 <- rbind(y_s, y_c) %>%
  mutate(Group=c(rep("Spurters", k), rep("Controls", k)))
  
# plot two distributions together
ggplot(plotdf2) +
  geom_point(aes(x=mu, y=sd, color=Group, shape=Group), 
             size=1,
             alpha=0.5) +
  ...
  ggtitle(label='Samples of (mu, sigma) from the posteriors for the two groups') +
  theme_light() + 
  NULL
...
- The spurters group tends to have higher posterior means of the change in IQ scores than the control group. 
- Also, the posteriors for the spurters group is more spread out (wider ranges of mu and sigma) because there is less data on the spurters than the controls.
...
```{r 4 Monte Carlo, echo=F, include=T}
k <- 10e6
# spurters
y_s <- NormalGamma(k, 24.0, 8, 4, 855.0)
# controls
...
p <- sum(y_s$mu > y_c$mu)/10e6
p

```
In 97% of all random samples of the posterior mean for the IQ score change in two groups, the spurters group has greater than mean change than the control group. 
...
```{r 5 check prior assumptions, echo=F, include=T}
k <- 10e2
# spurters
y_prior <- NormalGamma(k, 0, 1, 1/2, 10^2*1/2)

ggplot(y_prior) +
  geom_point(aes(x=mu, y=sd), size=1, color='darkgreen') +
  scale_x_continuous(limits=c(-50,50),
                     name='mu, mean change in IQ score') + 
  scale_y_continuous(limits=c(0,40),
                     name='lambda^(1/2), std. dev. of change in IQ score') +
  ggtitle(label='Samples of (mu, sigma) from the priors') +
  theme_light() + 
  NULL
...
- Findings:
- mu correctly centers around 0 as assumed.
- sigma (standard deviation) is higher when mu is further from 0.
